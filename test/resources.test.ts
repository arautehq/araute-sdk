...
async function setup<T>(
  ...
  run: (ctx: { calls: Array<{ url: string; init: RequestInit }>; client: Araute }) => Promise<T>,
): Promise<T> {
  const { calls, fetch } = recordingFetch(responseBody);
  return withMockFetch(fetch, async () => {
...
describe("invoices", () => {
  test("covers invoice actions and exact paths", async () => {
    await setup(invoice, async ({ calls, client }) => {
